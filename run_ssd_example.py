...
model_path = 'checkpoints/mb3-ssd-lite-Epoch-100-Loss-123.2478609085083.pth'
label_path = 'checkpoints/voc-model-labels.txt'
images_path = 'vision/datasets/soot/test_image'
...
for j in range(len(images_list)):
    image_path = os.path.join(images_path,images_list[j])
    ...
    for i in range(boxes.size(0)):
        ...
        cv2.putText(orig_image, label,
                    ...
                    (255, 0, 255),
                    2)  # line type
    path = os.path.join('result',images_list[j])
    cv2.imwrite(path, orig_image)
    print(f"Found {len(probs)} objects. The output image is {path}")
    end_time = time.time()
    cost1_time = end_time - start_time
    cost2_time = end_time - pre_time
    print(cost1_time)
    print(cost2_time)